...
export default function ContactClient() {
  ...
  return (
    <>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {infoCards.map((card, i) => (
              <motion.div key={card.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                ...
                className="rounded-3xl border border-border bg-card p-7 text-center transition-all hover:border-primary/25 hover:shadow-card hover:-translate-y-1">
                <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-green glow-green">
                  <card.icon className="h-6 w-6 text-primary-foreground" />
                </div>
                <h3 className="mb-2 font-heading font-bold text-foreground">{card.title}</h3>
                {card.lines.map(line => (
        ...
        <div className="container mx-auto px-4">
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            <motion.div ref={formRef} initial={{ opacity: 0, x: -30 }} animate={formInView ? { opacity: 1, x: 0 } : {}}
              ...
              <p className="mb-8 text-sm text-muted-foreground">Fill out the form and our team will get back to you within 24 hours.</p>
              ...
              {sent ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <input type="text" required placeholder="Your full name"
                        className="w-full rounded-xl border border-border bg-muted px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition-shadow" />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <select className="w-full rounded-xl border border-border bg-muted px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition-shadow">
                  ...
                  <div>
                    ...
                    <textarea required rows={5} placeholder="Write your message here..."
                      className="w-full resize-none rounded-xl border border-border bg-muted px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition-shadow" />
                  </div>
                  <button type="submit"
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-green px-6 py-3.5 font-heading font-semibold text-primary-foreground glow-green transition-all hover:scale-[1.02]">
            ...
            <motion.div ref={mapRef} initial={{ opacity: 0, x: 30 }} animate={mapInView ? { opacity: 1, x: 0 } : {}}
              ...
              <div className="min-h-[360px] flex-1 overflow-hidden rounded-3xl border border-border shadow-elevated">
                <iframe
              ...
              <div className="grid grid-cols-2 gap-4">
                <a href="tel:+2349070023282"
                  className="flex flex-col items-center gap-2 rounded-2xl border border-border bg-card p-5 text-center transition-all hover:border-primary/25 hover:shadow-card">
      ...
      <section className="bg-background py-16">
        <div className="container mx-auto px-4">
          <div className="mx-auto max-w-3xl rounded-3xl border border-border bg-card p-8 md:p-10">
            <h2 className="mb-6 font-heading text-2xl font-bold text-foreground">How to Find Us</h2>
            <div className="grid gap-6 md:grid-cols-3">
              {[
                ...
                <div key={d.method}>
                  <div className="mb-2 font-heading font-bold text-foreground">{d.method}</div>
